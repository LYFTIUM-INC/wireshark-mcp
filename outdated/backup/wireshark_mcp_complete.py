...
# ===== ADVANCED TOOL CLASSES (10 tools) =====
...
def format_advanced_tool_result(tool_name: str, result: Dict[str, Any]) -> str:
    """Format advanced tool results for display."""
    formatted = f"🦈 **{tool_name.replace('_', ' ').title()} Results**\n\n"
    
    if result.get("status") == "✅ Success":
        formatted += "✅ **Operation Successful**\n\n"
        
        # Remove status from display
        display_result = {k: v for k, v in result.items() if k != "status"}
        
        # Special formatting for certain tools
        if tool_name == "wireshark_pcap_split":
            if "created_files" in display_result:
                formatted += f"📁 **Created {display_result.get('file_count', 0)} files:**\n"
                for file in display_result.get('created_files', [])[:5]:
                    formatted += f"  - {file}\n"
                if len(display_result.get('created_files', [])) > 5:
                    formatted += f"  ... and {len(display_result['created_files']) - 5} more\n"
        else:
            # Default formatting
            formatted += "```json\n"
            formatted += json.dumps(display_result, indent=2)
            formatted += "\n```"
    else:
        # Error case
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    try:
        ...
        elif name.startswith("wireshark_pcap_") or name.startswith("wireshark_http_") or \
             name.startswith("wireshark_dns_") or name.startswith("wireshark_ssl_") or \
             name.startswith("wireshark_latency_") or name.startswith("wireshark_threat_") or \
             name.startswith("wireshark_hex_") or name.startswith("wireshark_remote_"):
            result = await handle_advanced_tool(name, arguments)
            return [TextContent(
                type="text",
                text=format_advanced_tool_result(name, result)
            )]
        else:
...
async def main():
    ...
    logger.info("🦈 Starting Complete Wireshark MCP Server v3.0 - All 18 Tools")
    logger.info("✨ Features: JSON Capture, Protocol Statistics, Enhanced Analysis")
    logger.info("📊 Total Tools Available: 18 (8 Original + 10 Advanced)")
